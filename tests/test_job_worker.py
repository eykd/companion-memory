...
@mock_aws
def test_worker_heartbeat_job_logging_paths() -> None:
    """Test that worker logs heartbeat jobs at various processing stages."""
    from unittest.mock import patch
    ...
    worker = JobWorker(job_table)
    worker.register_handler('heartbeat_event', TestJobHandler)

    now = datetime.now(UTC)

    # Test successful heartbeat job processing with logging
    ...
    job_table.put_job(heartbeat_job)

    with patch('companion_memory.job_worker.logger') as mock_logger:
        # Process the job - should succeed and log heartbeat-specific messages
        processed = worker.poll_and_process_jobs(now)
        assert processed == 1

        # Verify heartbeat dispatch logging
        mock_logger.info.assert_any_call(
            'HEARTBEAT JOB DISPATCH: job_id=%s, payload=%s, scheduled_for=%s',
            heartbeat_job.job_id,
            heartbeat_job.payload,
            heartbeat_job.scheduled_for.isoformat(),
        )


@mock_aws
def test_worker_heartbeat_job_failure_logging() -> None:
    """Test that worker logs heartbeat job failures with enhanced details."""
    from unittest.mock import patch
    ...
    # Create a handler that always fails
    class FailingHeartbeatHandler(BaseJobHandler):
        @classmethod
        def payload_model(cls) -> type[BaseModel]:
            return TestJobPayload

        def handle(self, payload: BaseModel) -> None:
            raise RuntimeError('Heartbeat processing failed!')

    worker = JobWorker(job_table)
    worker.register_handler('heartbeat_event', FailingHeartbeatHandler)

    now = datetime.now(UTC)
    heartbeat_job = ScheduledJob(
        job_id=uuid4(),
        job_type='heartbeat_event',
        payload={'message': 'test heartbeat'},
        scheduled_for=now - timedelta(minutes=1),
        status='pending',
        attempts=0,
        created_at=now,
    ...
    with patch('companion_memory.job_worker.logger') as mock_logger:
        # Process the job - should fail and log heartbeat-specific error
        processed = worker.poll_and_process_jobs(now)
        assert processed == 1

        # Verify heartbeat failure logging
        mock_logger.error.assert_any_call(
            'HEARTBEAT JOB FAILURE: job_id=%s, attempts=%d, error=%s, payload=%s',
            heartbeat_job.job_id,
            0,
            'RuntimeError: Heartbeat processing failed!',
            heartbeat_job.payload,
        )


@mock_aws
def test_worker_heartbeat_job_retry_logging() -> None:
    """Test that worker logs heartbeat job retry attempts."""
    from unittest.mock import patch

    job_table = JobTable()
    job_table.create_table_for_testing()

    # Create a handler that always fails (to trigger retry)
    class FailingHeartbeatHandler(BaseJobHandler):
        @classmethod
        def payload_model(cls) -> type[BaseModel]:
            return TestJobPayload
    ...
    worker = JobWorker(job_table, max_attempts=3)  # Allow retries
    worker.register_handler('heartbeat_event', FailingHeartbeatHandler)
    ...
    with patch('companion_memory.job_worker.logger') as mock_logger:
        # Process the job - should fail and schedule retry
        processed = worker.poll_and_process_jobs(now)
        assert processed == 1

        # Verify heartbeat retry logging
        retry_calls = [call for call in mock_logger.info.call_args_list if 'HEARTBEAT JOB RETRY' in str(call)]
        assert len(retry_calls) == 1


@mock_aws
def test_worker_heartbeat_job_dead_letter_logging() -> None:
    """Test that worker logs heartbeat jobs going to dead letter."""
    from unittest.mock import patch
    ...
    worker = JobWorker(job_table, max_attempts=1)  # No retries
    worker.register_handler('heartbeat_event', FailingHeartbeatHandler)
    ...
    with patch('companion_memory.job_worker.logger') as mock_logger:
        ...
        # Verify heartbeat dead letter logging
        mock_logger.error.assert_any_call(
